...
export default function CustomerMessageHistory() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: () => api.get('/customer/messages', { params: { ...filters, status: filters.status || undefined, phone: filters.phone || undefined, limit: 20 } }).then(r => r.data),
  });

  useSocket((event) => { if (event === 'message:status') refetch(); });
  ...
  return (
    <div>
      <div className="page-header">
        <h1 className="page-header-title">Lịch sử gửi tin</h1>
        <p className="page-header-desc">Tra cứu trạng thái và nhật ký các tin nhắn ZNS đã gửi</p>
      </div>
          ...
          <>
            <div className="table-wrapper">
              <table className="table">
                <thead><tr><th>Thời gian</th><th>Số điện thoại</th><th>Template</th><th>OA</th><th>Trạng thái</th><th>Mã lỗi</th></tr></thead>
                <tbody>
                  {data?.data?.map(m => (
                    <tr key={m.id}>
                  ...
                  {!data?.data?.length && <tr><td colSpan={6} className="empty-state"><div className="empty-state-title">Chưa có tin nhắn</div></td></tr>}
                </tbody>
              </table>
            </div>
            {data?.totalPages > 1 && (
